-- DevRect.lua
-- DevRect Library (Module) — v1.0 (KRNL Fixed)
local DevRect = {}
...
function DevRect:Create(titleText, descText, opts)
	...
	local scroll = newInstance("ScrollingFrame", {Parent = contentFrame, Size = UDim2.new(1, 0, 1, 0), ScrollBarThickness = 6, BackgroundTransparency = 1})
	...
	function self:Tab(name, iconId)
		...
		if index == 1 then stroke.Thickness = 2 end

		self.Tabs[#self.Tabs + 1] = tabObj
		tabObj._TabButton = tabBtn
		tabObj._ParentScroll = scroll
		function tabObj:SetBuilder(fn) tabObj._Builder = fn end
