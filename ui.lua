...
function DevRect.AddTab(name, callback)
	local info = {Name = name, Callback = callback}
	table.insert(tabs, info)
	CreateTab(info)
end

function DevRect.CreateButton(txt, parent, callback)
	return CreateButton(txt, parent, callback)
end

function DevRect.CreateToggle(txt, parent)
	return CreateToggle(txt, parent)
end

function DevRect.CreateSlider(name, parent)
	return CreateSlider(name, parent)
end

local function ShowNotification(title, message, duration, iconId)
...
function DevRect.ShowNotification(title, message, duration, iconId)
	ShowNotification(title, message, duration, iconId)
end

local UI_NAME = "DevRectUI"
local ICON_ID = 6031280882
...
strokeToggle.Color = Color3.fromRGB(120, 0, 0)
...
DevRect.notifContainer = notifContainer
DevRect.gui = gui
DevRect.main = main
DevRect.tabsFrame = tabsFrame
DevRect.contentFrame = contentFrame
DevRect.scroll = scroll

if #tabs > 0 then
	tabs[1].Callback(scroll)
	local firstTab = tabsFrame:FindFirstChildWhichIsA("TextButton")
	if firstTab then
		firstTab.BackgroundColor3 = Color3.fromRGB(45, 0, 0)
		local borderFrame = firstTab:FindFirstChildOfClass("Frame")
		if borderFrame then
			borderFrame.BorderColor3 = Color3.fromRGB(180, 0, 0)
		end
		DevRect.currentTab = firstTab
	end
end
